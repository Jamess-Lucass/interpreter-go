...
type IndexExpressopn struct {
	Token token.Token
	Left  Expression
	Index Expression
}

var _ Expression = (*IndexExpressopn)(nil)

func (s *IndexExpressopn) expressionNode() {}
func (s *IndexExpressopn) TokenLiteral() string {
	return s.Token.Literal
}
func (s *IndexExpressopn) String() string {
	var out bytes.Buffer
...
func (s *HashLiteral) TokenLiteral() string {
	return s.Token.Literal
}
func (s *HashLiteral) String() string {
	...
	for key, value := range s.Pairs {
		pars = append(pars, fmt.Sprintf("%s:%s", key.String(), value.String()))
	}

	out.WriteString("{")
	out.WriteString(strings.Join(pars, ", "))
	out.WriteString("}")
